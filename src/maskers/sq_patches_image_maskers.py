...
class SqPatchesImageMasker(ImageMasker):
    ...
    @abstractmethod
    def _get_patch_iteration_data(self, masking_results: pd.DataFrame, filtered_masking_results: pd.DataFrame, rng: np.random.Generator | None = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]: pass
    ...
    def _perform_masking(self, img_tensor: torch.Tensor, mr: float, masking_results: pd.DataFrame, filtered_masking_results: pd.DataFrame, masked_patches: List[str], rng: np.random.Generator | None = None):
        ...
        coords, patch_ids, pick_order = self._get_patch_iteration_data(masking_results, filtered_masking_results, rng)
    ...
    def mask_instance(self, instance_path: str, instance_name: str):
        ...
        for mr in self.mask_rates:
            ...
            img_tensor, masked_area, masked_patches = self._perform_masking(img_tensor, mr, masking_results, filtered_masking_results, masked_patches, rng)
...
class SqPatchesSaliencyImageMasker(SqPatchesImageMasker):
    ...
    def _get_patch_iteration_data(self, masking_results: pd.DataFrame, filtered_masking_results: pd.DataFrame, rng: np.random.Generator | None = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
class SqPatchesRandomImageMasker(SqPatchesImageMasker):
    ...
    def _build_instance_rng(self, instance_name: str) -> np.random.Generator:
        assert self.global_seed is not None, "Inside 'SqPatchesRandomImageMasker' 'self.global_seed' must not be 'None'!"
    ...
    def _get_patch_iteration_data(self, masking_results: pd.DataFrame, filtered_masking_results: pd.DataFrame, rng: np.random.Generator | None = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        assert rng is not None, "Inside 'SqPatchesRandomImageMasker' 'rng' cannot be 'None'!"
        ...
        return coords, patch_ids, rng.permutation(len(coords))
